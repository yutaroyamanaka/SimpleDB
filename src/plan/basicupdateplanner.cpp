...
namespace plan {
  ...
  int BasicUpdatePlanner::executeDelete(const parse::DeleteData& data, tx::Transaction* transaction) {
  ...
  int BasicUpdatePlanner::executeModify(const parse::ModifyData& data, tx::Transaction* transaction) {
    auto p = std::static_pointer_cast<Plan>(std::make_shared<TablePlan>(transaction, data.tableName(), mdm_));
    p = std::static_pointer_cast<Plan>(std::make_shared<SelectPlan>(p, data.pred()));
    auto us = std::static_pointer_cast<scan::UpdateScan>(p->open());
    int count = 0;
  ...
  int BasicUpdatePlanner::executeInsert(const parse::InsertData& data, tx::Transaction* transaction) {
    auto p = std::static_pointer_cast<Plan>(std::make_shared<TablePlan>(transaction, data.tableName(), mdm_));
    auto us = std::static_pointer_cast<scan::UpdateScan>(p->open());
    us->insert();
    auto vals = data.vals();
  ...
  int BasicUpdatePlanner::executeCreateTable(const parse::CreateTableData& data, tx::Transaction* transaction) {
    mdm_->createTable(data.tableName(), data.newSchema(), transaction);
    return 0;
  }

  int BasicUpdatePlanner::executeCreateView(const parse::CreateViewData& data, tx::Transaction* transaction) {
    mdm_->createView(data.viewName(), data.viewDef(), transaction);
    return 0;
  }

  int BasicUpdatePlanner::executeCreateIndex(const parse::CreateIndexData& data, tx::Transaction* transaction) {
