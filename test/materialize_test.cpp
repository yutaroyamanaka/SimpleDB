...
TEST(MaterializeTest, Main) {
  ...
  try {
    //std::string qry = "select sname, gradyear, grade from ( select sid, sname from student where gradyear = 2005 ) , ( select studentid from enroll where grade = 'A' ) where sid = studentid";
